...
def check_score_button(stats, score_button, mouse_x, mouse_y):
    if score_button.rect.collidepoint(mouse_x, mouse_y):
        stats.score_active = True


def check_events(stats, play_button, score_button, ship, sound):
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT: sys.exit()
            ...
            check_score_button(stats=stats, score_button=score_button, mouse_x=mouse_x, mouse_y=mouse_y)
